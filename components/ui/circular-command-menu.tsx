...
function Component({
  ...
  // Responsive radius: if `responsive` is true, shrink radius on small screens
  useEffect(() => {
    if (!responsive) {
    ...
    const update = () => {
      ...
      if (w < 640) setComputedRadius(Math.max(48, Math.round(radius * 0.5)))
      else if (w < 1024) setComputedRadius(Math.max(80, Math.round(radius * 0.75)))
      else setComputedRadius(radius)
    }
  ...
  const getItemPosition = (index: number) => {
    ...
    return {
      x: Math.cos(angle) * computedRadius,
      y: Math.sin(angle) * computedRadius,
    }
  }
